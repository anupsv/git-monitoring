...
import (
	...
	mockgithub "github.com/asv/git-monitoring/pkg/tools/common/test"
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "Error fetching reviews" {
				// Explicitly set the MockReviewErr for this test case
				mockClient.ListPullRequestReviewsFunc = func(ctx context.Context, owner, repo string, number int, opts *github.ListOptions) ([]*github.PullRequestReview, *github.Response, error) {
					return nil, nil, tc.mockReviewError
				}
				...
				// Explicitly handle other cases that need specific review behavior
				mockClient.ListPullRequestReviewsFunc = func(ctx context.Context, owner, repo string, number int, opts *github.ListOptions) ([]*github.PullRequestReview, *github.Response, error) {
					return tc.mockReviews, &github.Response{NextPage: 0}, nil
				}
			}
			...
			result := service.CheckRepository(tc.repository, "test-token", tc.timeWindow, false)
